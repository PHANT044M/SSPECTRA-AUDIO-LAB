...
def resource_path(relative_path):
    """Возвращает абсолютный путь к ресурсам. Работает как в режиме исходного кода, так и внутри скомпилированного .exe."""
    try:
...
if __name__ == "__main__":
    # Заставляем Windows корректно отображать кастомную иконку на панели задач
    try:
        import ctypes
        myappid = f"phant044m.spectra.audiolab.{APP_VERSION}"
    ...
    # Безопасная загрузка иконки из внутренних ресурсов .exe
    try:
        icon_path = resource_path("icon.ico")
        if os.path.exists(icon_path):
            root.iconbitmap(icon_path)
            root.wm_iconbitmap(icon_path)  # Двойной вызов для надежности на разных версиях Windows
        else:
            icon_png_path = resource_path("icon.png")
            if os.path.exists(icon_png_path):
                img = tk.PhotoImage(file=icon_png_path)
                root.iconphoto(True, img)  # Современный кроссплатформенный метод для PNG
    except Exception:
        pass
