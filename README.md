Самый удобный и профессиональный способ оформить проект для международной и русскоязычной аудитории на GitHub — сделать **двуязычный `README.md` в одном файле**. 

В самом верху мы разместим быстрое меню навигации `[English] | [Русский]`, а далее расположим два полноценных, детально расписанных блока документации. Это стандарт для крупных репозиториев, так как пользователям не нужно переключаться между разными файлами.

Вы можете полностью скопировать этот готовый код и вставить его в ваш файл `README.md`:
